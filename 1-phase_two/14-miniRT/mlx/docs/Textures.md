...
disk into our memory and store the information in a `mlx_texture*`. After that we create a new `t_mlx_image*` 
...
int32_t	main(void)
{
	...
	t_mlx_texture* texture = mlx_load_png("./temp/sus.png");
	if (!texture)
        error();
	
	// Convert texture to a displayable image
	t_mlx_image* img = mlx_texture_to_image(mlx, texture);
	if (!img)
        error();
