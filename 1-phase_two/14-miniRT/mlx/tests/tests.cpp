...
TEST(MWindow, Settings)
{
	...
	t_mlx *mlx = mlx_init(400, 400, "MLX42", false);
...
{
	t_mlx_image* img = mlx_new_image(mlx, Window::width / 2, Window::height / 2);
...
{
	t_mlx_image* img1 = mlx_new_image(mlx, Window::width / 2, Window::height / 2);
	ASSERT_NE(img1, nullptr);
	ASSERT_EQ(g_mlx_errno, MLX_SUCCESS);

	t_mlx_image* img2 = mlx_new_image(mlx, Window::width, Window::height);
...
static void ft_draw(void* param)
{
	...
	static t_mlx_image* img = nullptr;
